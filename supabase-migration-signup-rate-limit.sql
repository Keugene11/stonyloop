-- Emergency: add a global signup burst limit to the handle_new_user trigger.
-- Threshold: reject any signup if >= 5 signups occurred in the previous 30 seconds.
-- Rationale: even a packed orientation-day campus rate stays well under 10/min.
...
BEGIN
  ...
  -- Global burst limit: cap signups to 5 per rolling 30 seconds.
  SELECT count(*) INTO recent_count
  FROM auth.users
  WHERE created_at > now() - interval '30 seconds';

  IF recent_count >= 5 THEN
    RAISE EXCEPTION 'signup_rate_limited: too many signups in the last 30 seconds'
      USING ERRCODE = '22023';
  END IF;
